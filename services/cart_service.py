...
from utils.exceptions import CartNotFound, DataBoundaryViolation, CartException
...
class CartService:
    ...
    def get_active_cart(db: Session, user_id: int):
        try:
            ...
            if not cart:
                log_user_activity(user_id, "Failed to retrieve cart: Active session not found.", "error")
                raise CartNotFound()
    ...
    def create_new_cart(db: Session, user_id: int):
        try:
            ...
            if existing_cart:
                log_user_activity(user_id, "Aborted cart creation: Active cart already exists.", "error")
                raise CartException(detail="You already have an active cart session.")
            
            # 2. Attempt creation
            new_cart = CartRepository.create_cart(db, user_id)
            log_user_activity(user_id, f"Successfully created new active cart ID: {new_cart.id}", "info")
            return new_cart
    ...
    def add_item(db: Session, user_id: int, product_id: int, quantity: int) -> Cart:
        ...
        if not cart:
            ...
            log_user_activity(user_id, f"Auto-initialized fresh active cart session ID: {cart.id}", "info")

        product = CartRepository.get_product(db, product_id)
        if not product:
            log_user_activity(user_id, f"Failed to add item: Product ID {product_id} not found in database catalog.", "error")
        ...
        if existing_item:
            ...
            db.commit() 
            log_user_activity(user_id, f"Aggregated quantity: product_id={product_id} increased by count={quantity} in cart_id={cart.id}", "info")
        else:
            ...
            log_user_activity(user_id, f"Added new product_id={product_id} snapshot price={product.current_price} to cart_id={cart.id}", "info")
    ...
    def remove_item(db: Session, user_id: int, product_id: int) -> Cart:
        ...
        log_user_activity(user_id, f"Removed product_id={product_id} from cart_id={cart.id}", "info")
        db.refresh(cart)
        return cart
    ...
    def checkout(db: Session, user_id: int) -> Cart:
        ...
        db.commit()
        log_user_activity(user_id, f"Cart ID {cart.id} successfully CONVERTED (Checked out)", "info")
        return cart
    ...
    def delete_cart(db: Session, user_id: int):
        ...
        log_user_activity(user_id, f"Successfully deleted active cart session ID: {cart_id}", "info")