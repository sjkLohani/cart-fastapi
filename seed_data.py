...
def seed_database():
    ...
    with engine.begin() as connection:
        ...
        min_p = connection.execute(select(func.min(tables['products'].c.id))).scalar() or 1
        max_p = connection.execute(select(func.max(tables['products'].c.id))).scalar() or 100000
        ...
        max_c = connection.execute(select(func.max(tables['carts'].c.id))).scalar() or 300000
